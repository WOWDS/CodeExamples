...
class Login {
	...
	private $user_email														= "";
	// @var string $user_type_id The user's login type
	...
	*/
	public function __construct(){
		...
		// function below uses $_SESSION['user_id'] and $_SESSION['user_email']
		$this->editUserName($_POST['user_name']);
		// user try to change his email
		} elseif (isset($_POST["user_edit_submit_email"])){
		// function below uses $_SESSION['user_id'] and $_SESSION['user_email']
		$this->editUserEmail($_POST['user_email']);
		// user try to change desktop preferences
		} elseif (isset($_POST["user_edit_submit_desktop"])){
	...
	*/
	private function loginWithSessionData(){
		...
		$this->user_type_id													= $_SESSION['UTID'];
		...
		if (isset($_COOKIE['rememberme'])){
			...
			if ($hash == hash('sha256', $user_id . ':' . $token . COOKIE_SECRET_KEY) && !empty($token)){
				...
				if ($this->databaseConnection()){
					...
					$sth																		= $this->db_connection->prepare("SELECT user_id, user_name, user_email, user_type_id, user_last_login FROM users WHERE user_id = :user_id AND user_rememberme_token = :user_rememberme_token AND user_rememberme_token IS NOT NULL");
					$sth->bindValue(':user_id', $user_id, PDO::PARAM_INT);
					$sth->bindValue(':user_rememberme_token', $token, PDO::PARAM_STR);
					$sth->bindValue(':user_last_login', $user_last_login, PDO::PARAM_STR);
					$sth->execute();
					// get result row (as an object)
					...
					if (isset($result_row->user_id)){
						...
						$_SESSION['UTID']										= $result_row->user_type_id;
						$_SESSION['user_last_login']				= $result_row->user_last_login;
						if ($_SESSION['user_last_login'] <> '0000-00-00 00:00:00'){
							$_SESSION['LastLog']							= date('jS F Y', strtotime($_SESSION['user_last_login']));
						} else {
							$_SESSION['LastLog']							= 'First Login';
						}
						...
						$this->user_type_id									= $result_row->user_type_id;
						$this->user_last_login								= $result_row->user_last_login;
						$this->user_is_logged_in							= true;
	...
	private function loginWithPostData($user_name, $user_password, $user_rememberme){
		if (empty($user_name)){
			...
			if (! isset($result_row->user_id)){
				...
				// reset the failed login counter and updater last login date for that user
				$sth																			= $this->db_connection->prepare('UPDATE users SET user_failed_logins = 0, user_last_failed_login = NULL, user_last_login = "'.$Today	.'" WHERE user_id = :user_id');
	...
	*/
	public function setPasswordResetDatabaseTokenAndSendMail($user_name){
		...
		if (empty($user_name)){
			...
			if (isset($result_row->user_id)){
				// database query:
				$query_update														= $this->db_connection->prepare('UPDATE users SET user_password_reset_hash = :user_password_reset_hash, user_password_reset_timestamp = :user_password_reset_timestamp WHERE user_name = :user_name');
	...
	*/
	public function editNewPassword($user_name, $user_password_reset_hash, $user_password_new, $user_password_repeat){
		...
		if (empty($user_name) || empty($user_password_reset_hash) || empty($user_password_new) || empty($user_password_repeat)){
			...
			$user_password_hash											= password_hash($user_password_new, PASSWORD_DEFAULT, array('cost' => $hash_cost_factor));
			// write users new hash into database
			$query_update															= $this->db_connection->prepare('UPDATE users SET user_password_hash = :user_password_hash, user_password_reset_hash = NULL, user_password_reset_timestamp = NULL WHERE user_name = :user_name AND user_password_reset_hash = :user_password_reset_hash');
			$query_update->bindValue(':user_password_hash', $user_password_hash, PDO::PARAM_STR);
			$query_update->bindValue(':user_password_reset_hash', $user_password_reset_hash, PDO::PARAM_STR);
			$query_update->bindValue(':user_name', $user_name, PDO::PARAM_STR);
			$query_update->execute();
