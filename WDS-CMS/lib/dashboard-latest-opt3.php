<?
	...
	<h3>News &amp; Events <span class="le icons"></span></h3>';
	$Op4Q															= $mysqli->query("SELECT PID, PTID, Title, DATE_FORMAT(PDate, '%D %M %Y') AS PostD FROM Posts WHERE PTID = '9' ORDER BY PDate DESC LIMIT 3");
	$O4N																= $Op4Q->num_rows;
	$o4i																= 0;
	if ($O4N <> 0) {
